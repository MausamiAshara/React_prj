...
const BlockedBrokers = () => {
  ...
  const changeBlockBroker = (value) => {
    const data = {
      query: value.trim(),
      user_type: 'BR'
    }
    dispatch(searchBlockedUser(data)).then(res => setInitialUser(res.data)).catch(err => brokerListing())
  }
      ...
      <div className='patients-search'>
        <div className='custom-search'>
          ...
          <DebounceInput
            ...
            onChange={(e) => changeBlockBroker(e.target.value)}
          />
        </div>
      </div>
      <Table
