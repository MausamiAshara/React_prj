...
const BlockedFunders = () => {
  ...
  const changeBlockFunder = (value) => {
    const data = {
      query: value.trim(),
      user_type: 'FU'
    }
    dispatch(searchBlockedUser(data)).then(res => setInitialUser(res.data)).catch(err => funderListing())
  }
      ...
      <div className='patients-search'>
        <div className='custom-search'>
          ...
          <DebounceInput
            ...
            onChange={(e) => changeBlockFunder(e.target.value)}
          />
        </div>
      </div>
      <Table
