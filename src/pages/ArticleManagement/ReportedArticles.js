...
const ReportedArticles = () => {
  ...
  const showViewModal = (record) => {
    dispatch(getArticleInfo(record))
      .then((response) => {
        if (response?.status === 200) {
          setViewModalRecord(response?.data)
        }
      }).catch((error) => (error?.message))
  ...
  const showDeleteModal = (record) => {
    ...
    dispatch(getArticleInfo(record))
      .then((response) => {
        setReportedArticleId(response?.data?.id)
      }).catch((error) => error?.message)
  }
  ...
  const changeReportedArticle = (value) => {
    const data = {
      query: value.trim(),
    }
    dispatch(searchReportedArticle(data)).then(res => setReportedArticle(res.data)).catch(err => reportedArticleList())
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row, record) => <div>
        <div>{record?.title.length > 20 ? record?.title.slice(0, 25) + "..." : record?.title}</div>
      </div>
    },
    {
      title: 'Description',
      ...
      <div className='patients-search'>
        <div className='custom-search'>
          ...
          <DebounceInput
            ...
            onChange={(e) => changeReportedArticle(e.target.value)}
          />
        </div>
      </div>
      ...
      >
        <div className='funder-detail'>
          <ul>
            ...
            <li>
              <span className='left'>Description</span>{viewModalRecord?.description?.length > 20 ?
                <textarea className='right' cols={50} rows={3} value={viewModalRecord?.description} disabled />
                :
            ...
            <li>
              ...
              <span className='right'>
                {viewModalRecord?.reported_by?.length > 2 ?
                  <textarea cols={35} rows={3} value={viewModalRecord?.reported_by.map((item) => " " + item?.first_name)} disabled />
                  :
                  viewModalRecord?.reported_by.map((item) => <>{item?.first_name}</>)
                }
              </span>
            </li>
          </ul>
            ...
            {viewModalRecord?.article_media?.map((item) => {
              return (
                <a href={item?.article_media} target="_blank" rel='noreferrer'>
                  <img src={item?.article_media ? item?.article_media : toAbsoluteUrl('/images/user-img.svg')} title={item?.article_media ? "" : "not found"} alt="" width={450} height={250} />
                </a>
              )
            })}
          </div>
          ...
          <div className='post-detail'>
            <div className='footer-post'>
              <span className='like'><LikeIcon />{viewModalRecord?.likes}</span>
              <span className='comment'><CommentIcon />{viewModalRecord?.comment}Comments</span>
            </div>
          </div>
